...
const Draw = () => {
  ...
  useEffect(() => {
    const fetchDrawResults = async () => {
      setLoading(true);
      try {
        const res = await axios.get("https://app.afriluck.com/api/V1/app/draw-results", {
          headers: {
  ...
  return (
    <div className="flex flex-col bg-[#F7F7F7] mx-4 md:mx-12 lg:mx-48">
      {error && <p className="h-full text-wrap p-5 text-center text-black">{error}</p>}
        ...
        {loading ? (
          ...
          Object.entries(groupedResults).map(([day, draws]) => (
            <div key={day} className="mb-6">
              <h2 className="font-Poppins text-lg font-normal text-white px-6 py-2 bg-[#156064] rounded-md">{day}</h2>
              {draws.map((result, index) => (
                <div key={index} className="bg-white rounded-xl h-auto w-full p-6 my-3 border border-border-default">
